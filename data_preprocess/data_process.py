...
# 合并三个原始表
# 读取三个Excel文件
df1 = pd.read_excel("output/big_data1.xlsx", engine='openpyxl')
df2 = pd.read_excel("output/big_data2.xlsx", engine='openpyxl')
...
df = df1.merge(df2, on="文件名", how='inner',suffixes=("", '_df2'))
df['文件名'] = df['文件名'].apply(lambda x: x.split('/')[-1])
df['日期'] = df['日期'].apply(lambda x: x.split('/')[-1])
df['年份'] = df['年份'].apply(lambda x: x.replace('年', ''))
...
new_name_list = {
    ...
    '研发人员占职工人数比例': {'公式': '研发人员占职工人数比例=研发人员人数/职工总人数', '数值': ['研发人数', '职工总数']},
    '企业硕士及以上人员占职工人数比例': {'公式': '企业硕士及以上人员占职工人数比例=(硕士员工人数 + 博士及以上的员工人数)/职工总人数', '数值': ['硕士人员', '博士及以上人员', '职工总数']},
    '毛利率': {'公式': '毛利率=(营业收入-营业成本)/营业收入', '数值': ['营业收入', '营业成本']},
    '营业利润率': {'公式': '营业利润率=营业利润/营业收入', '数值': ['营业利润', '营业收入']},
    '流动比率': {'公式': '流动比率=流动资产/流动负债', '数值': ['流动资产合计', '流动负债合计']},
    '速动比率': {'公式': '速动比率=(流动资产-存货)/流动负债', '数值': ['流动资产合计', '存货', '流动负债合计']},
    '资产负债比率': {'公式': '资产负债比率=总负债/资产总额', '数值': ['负债合计', '资产总计']},
    '现金比率': {'公式': '现金比率=货币资金/流动负债', '数值': ['货币资金', '流动负债合计']},
    '非流动负债比率': {'公式': '非流动负债比率=非流动负债/总负债', '数值': ['非流动负债合计', '负债合计']},
    '流动负债比率': {'公式': '流动负债比率=流动负债/总负债', '数值': ['流动负债合计', '负债合计']},
...
for index, row in tqdm(df.iterrows()):
    ...
    #
    #     # 1. 计算2020和2019的货币资金
    #     currency_this = df[(df['公司名称'] == company_name) & (df['年份'] == year)][name].sum()
    #     currency_last = df[(df['公司名称'] == company_name) & (df['年份'] == year-1)][name].sum()
    #
    #     print(currency_this)
    #     print(currency_last)
    #
    ...
    for new_name in new_name_list:
        ...
        for sz_name in new_name_list[new_name]['数值']:
            currency_this = str(df[(df['公司名称'] == company_name) & (df['年份'] == year)][sz_name].sum())
            shuzhi_dict[sz_name] = currency_this
            gongshi = gongshi.replace(sz_name, currency_this)

        try:
            gongshi = gongshi.replace(",",'')
            growth_rate_this = str(round(eval(gongshi),2))
...
# 保存结果
df.to_excel("output/big_data.xlsx", engine='openpyxl', index=False)